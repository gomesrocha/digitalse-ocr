...
public static String[] token(String text) {
		...
		try {
			mi = new FileInputStream("data/pt-token.bin");
			model = new TokenizerModel(mi);
		...
		String tokens[] = tokenizer.tokenize(te.extract(imageFile, "por"));
        ...
        Tokenizer tk = WhitespaceTokenizer.INSTANCE;
        String tks[] = tk.tokenize(te.extract(imageFile, "por"));
 
        System.out.println("Token\n----------------");
        for(int i=0;i<tks.length;i++){
            System.out.println(tks[i]);
        }
		//System.out.println(te.extract(imageFile, "por"));
